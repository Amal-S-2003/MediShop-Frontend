...
const ViewOrdersPage = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      {orders.length > 0 ? (
        orders.map((order) => (
          <div key={order._id} className="border rounded-lg p-4 shadow-md bg-white mb-4">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-semibold text-gray-700">Order ID: {order._id}</h3>
              <span className={`px-3 py-1 text-sm rounded ${order.status === "Delivered" ? "bg-green-200 text-green-700" : "bg-yellow-200 text-yellow-700"}`}>{order.status}</span>
            </div>
              ...
              <div className="flex items-center justify-between w-full">
                {statusSteps.map((step, index) => (
                  <div key={index} className="flex flex-col items-center w-1/5 text-center">
                    <div
                      className={`w-8 h-8 flex items-center justify-center rounded-full text-white font-semibold ${order.status === step ? "bg-blue-500" : statusSteps.indexOf(order.status) > index ? "bg-green-500" : "bg-gray-300"}`}
                    >
                      {statusSteps.indexOf(order.status) > index ? "✓" : index + 1}
                    </div>
                    <span className={`text-sm mt-1 ${order.status === step ? "text-blue-600" : "text-gray-500"}`}>{step}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {expandedOrder === order._id && (
              <div className="mt-4 border-t pt-4">
                  ...
                  <div key={index} className="flex justify-between items-center border-b pb-2 mt-2">
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <p className="text-gray-500 text-sm">₹{product.price} x {product.quantity}</p>
                      </div>
                    </div>
                    <p className="font-semibold text-green-600">₹{product.total}</p>
                  </div>
                ))}
                <div className="flex justify-between text-gray-600 mt-2">
                  <span>Subtotal:</span>
                ...
                <div className="flex justify-between text-gray-600">
                  ...
                  <span className="font-medium text-red-500">₹{order.shippingFee.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-gray-800 text-xl font-bold">
                  <span>Total:</span>
                  <span className="text-green-600">₹{order.totalPrice.toFixed(2)}</span>
